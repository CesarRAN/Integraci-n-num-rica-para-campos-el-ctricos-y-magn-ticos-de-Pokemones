...
def campo_vectorial_1(x, y, z):
    ...
    """
    if np.sqrt((x)**2 + y**2 + (z**2)) < 1 and z>0:
        return 1
    if np.sqrt((x)**2 + y**2 + (z**2)) < 1 and z<0:
        return -1
    else:
...
def campo_vectorial_2(x, y, z, v):
    ...
    Modifica las expresiones según el campo que desees representar.
    """
    condicion_1 = np.sqrt(x**2 + y**2 + (z**2)) < 1 
    condicion_2 = z>0
    
    f_x = np.where(condicion_1, np.where(condicion_2, v, -v), 0)  # (0 si condición, -y si no)
    f_y = np.where(condicion_1, np.where(condicion_2, v, -v), 0)     # (0 si condición, x si no)
    f_z = np.where(condicion_1, np.where(condicion_2, v, -v), 0)     # (0 si condición, z si no)
    return f_x, f_y, f_z
...
def graficar_campo_vectorial(campo_func, rango=(-2, 2), densidad=10, R=1):
    ...
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title(f'Campo Magnetico, Malla: {densidad}',fontsize=30)
...
graficar_campo_vectorial(calcular_campo_magnetico, rango=(-5, 5), densidad=12)
